...
[![Research](https://img.shields.io/badge/Type-Research%20PoC-8b5cf6?style=for-the-badge)](RESEARCH_RETROSPECTIVE.md)
[![Fuzzing](https://img.shields.io/badge/Fuzzing-4.3B%2B%20exec-10b981?style=for-the-badge)](#-fuzzing-results)
[![Upstream](https://img.shields.io/badge/Upstream-svm%2325%20closed-ef4444?style=for-the-badge)](https://github.com/anza-xyz/svm/issues/25)
...
📄 **[Research Retrospective](RESEARCH_RETROSPECTIVE.md)** — full narrative of the research cycle, upstream engagement (svm#25), and architectural conclusions.

---

## ⚠️ Important Update (August 2026)

The `snapshot.entries.clear()` bug described below was **present only in this PoC**. The official `anza-xyz/agave-runtime/feat/abiv2` uses `abi_v2_prepare_for_instruction()` with explicit `make_immutable()`, which correctly resets permissions at every instruction boundary. See the [Research Retrospective](RESEARCH_RETROSPECTIVE.md) for the full upstream engagement timeline.

This repository is preserved as a **research artifact** documenting the fuzzing methodology (stateful invariant checking, 4.3B+ executions) and the CoreState accounting module.
...
- **Archive the permission-rollback PoC** — the official `agave-runtime/feat/abiv2` has superseded this approach with `abi_v2_prepare_for_instruction()` + `make_immutable()`.
- **Preserve the fuzzing methodology** — the 4.3B-execution invariant harness and CoreState module remain valuable for future research on other Solana runtime components.
- **Document the research cycle** — see [RESEARCH_RETROSPECTIVE.md](RESEARCH_RETROSPECTIVE.md) for the full upstream engagement narrative.
- **Continue research** — focusing on upstream `agave-runtime` components (instruction account deduplication, VM error paths, scheduler internals) with tests against the actual codebase.
...
| `RESEARCH_RETROSPECTIVE.md` | Research cycle documentation | This repository |
